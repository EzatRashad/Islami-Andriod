...
class HadethFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        snapHelper.attachToRecyclerView(binding.hadethListRecyclerView)
    }

    private fun readAhadethFile(): List<Hadeth> {
        val content = requireContext().assets.open("hadeth/ahadeth.txt").bufferedReader().use { it.readText() }
        ...
        for (block in hadethBlocks) {
            ...
            if (lines.size > 1) {
                ...
                hadethList.add(Hadeth(title, description))
            }
        }
        return hadethList
    }
